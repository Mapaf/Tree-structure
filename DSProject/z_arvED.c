...
TipoNoArv* no_remove(TipoNoArv *a){
	...
	if(a->esq == NULL){ // tem zero ou um filho à direita
		atual = a->dir;
		free(a);
		return atual;
	} else { // tem dois filhos ou um filho à esquerda
	    ant = a;
		atual = a->esq;
...
/*
   Versão genérica de percurso (em ordem) e visita
   Rotina faz o Call Back da função passada "visita" 
   sobre cada registro armazenado da árvore
   USA TIPO: typedef int (*visitador)(TipoRegistro);
   DE:  registro.h
*/
void ed_arv_visita(TipoNoArv* raiz, visitador mf){
    if(raiz != NULL){
      ed_arv_visita(raiz->esq, mf);
      (*mf)(raiz->reg);
      ed_arv_visita(raiz->dir, mf);
   }
}

/*
   Versão genérica de percurso (em ordem) e modificação
   Rotina faz o Call Back da função passada para modificar
   cada registro armazenado da árvore
   
   USA TIPO: typedef int (*mapeador)(&TipoRegistro);
   DE:  registro.h
*/
void ed_arv_mapeia(TipoNoArv* raiz,  mapeador mf){
    if(raiz != NULL){
      ed_arv_mapeia(raiz->esq, mf);
      (*mf)(&(raiz->reg)); // modifica registro
      ed_arv_mapeia(raiz->dir, mf);
   }
}
...
int ed_arv_busca(int chave, TipoNoArv *atual, TipoRegistro *reg){

	while(atual != NULL){ // começa na raiz
      if(atual->reg.chave == chave){
       (*reg) = atual->reg; // retorna registro por referência
	   return 1;
      } else {
